...
<script>
	function confirmComplete() {
		return answer = confirm("¿Estás seguro de que deseas eliminar esta categoría?"
						...
						<form action="eliminarCategoria" method="post"
							...
							<table id="listado_tareas" class="table table-bordered table-hover">
								...
								<c:forEach var="task" items="${listaMostrar}" varStatus="i">
									<tr id="task_${task.id}">
										<td><label class="btn btn-default"> <input
												type="radio" name="tareaId" value="${task.id}" /></label>
										<td>${task.title}</td>
										<td>${task.created}</td>
										<c:choose>
							...
							<div class="container">
								<center>
									<c:if test="${ultimaLista!='Hoy'}">
										<a href="cambiarFinalizadas" id="mostrar_finalizadas" 
											class="btn btn-info navbar-btn">
										<c:if test="${mostrarFinalizadas}">
											No mostrar finalizadas
										</c:if> <c:if test="${not mostrarFinalizadas}">
											Mostrar finalizadas
											</c:if>
										</a>
									...
									<button type="button" class="btn btn-success navbar-btn"
										id="crear_tarea" data-toggle="modal" data-target="#crearTarea">Crear
										tarea</button>
									<button type="submit" class="btn btn-warning navbar-btn"
										id="editar_tarea" onclick="form.action='prepararEdicionTarea'">Editar
										tarea</button>
									<button type="submit" class="btn btn-primary navbar-btn"
										id="finalizar_tarea" onclick="form.action='marcarFinalizada'">Finalizar
										tarea</button>
									<div class="btn-group">&nbsp | &nbsp</div>
									<button type="button" class="btn btn-success navbar-btn"
										id="crear_categoria" data-toggle="modal" data-target="#crearCategoria">Crear
										categoría</button>
									<button type="submit" class="btn btn-warning navbar-btn"
										id="modificar_categoria" onclick="form.action='menuModificarCategoria'">Modificar
										categoría</button>
									<button type="submit" class="btn btn-primary navbar-btn"
										id="duplicar_categoria" onclick="form.action='duplicarCategoria'">Duplicar
										categoría</button>
									<button type="submit" class="btn btn-danger navbar-btn"
										id="eliminar_categoria" onclick="return confirmComplete();">Eliminar
										categoría</button>
								</center>
							</div>
							</nav>
				...
				<div class="modal-content">
					...
					<form action="crearTarea" method="post" name="crearTarea_form_name">
						...
						<div class="modal-footer">
							<button id="cerrar_tarea" class="btn btn-default" data-dismiss="modal">Cerrar</button>
				...
				<div class="modal-content">
					...
					<form action="crearCategoria" method="post"
						...
						<div class="modal-footer">
							...
							<input id="crear_categoria2" type="submit" class="btn btn-success"
