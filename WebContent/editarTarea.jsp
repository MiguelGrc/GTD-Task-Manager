...
<body>
	<form action="editarTarea" method="post" name="crearTarea_form_name" id="crearTarea_form">
		...
		<table align="center">
			...
			<tr>
				<td><div class="form-group">
						<label for="fecha">Fecha planeada</label> <input type="text"
							class="form-control" name="fechaPlaneadaTarea"
							id="fecha" placeholder="dd/mm/yyyy" size="30"
			...
			<tr>
				<td><div class="form-group">
						...
						<select name=categoria class="form-control" from="crearTarea_form">
						<c:forEach var="categoria" items="${categorias}" varStatus="i">
							<option <c:if test="${categoria.id==categoriaSeleccionada}">selected
								</c:if> value="${categoria.id}">${categoria.name}</option>
						</c:forEach>
					</select>
				</div></td>
			</tr>
